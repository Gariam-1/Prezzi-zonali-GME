...
def extract_xml(archive: ZipFile, pun_data: PricesData, pz_data: PricesData, zone: str) -> PricesData:
    ...
    for fascia in pun_data.data.values():
        fascia.clear()
        fascia = [0] * 24
    for fascia in pz_data.data.values():
        fascia.clear()
        fascia = [0] * 24
    ...
    for fn in sorted(filelist):
        ...
        for prezzi in xml_root.iter("Prezzi"):
            ...
            prezzo_pun_string = prezzi.find("PUN").text
            prezzo_pun_string = prezzo_pun_string.replace(".", "").replace(",", ".")
            prezzo_pun = float(prezzo_pun_string) / 1000

            # Estrae il prezzo zonale dall'XML in un float
            prezzo_pz_string = prezzi.find(zone).text
            prezzo_pz_string = prezzo_pz_string.replace(".", "").replace(",", ".")
            prezzo_pz = float(prezzo_pun_string) / 1000

            # Estrae la fascia oraria
            fascia = get_fascia_for_xml(dat_date, festivo, ora)

            # Somma tutti i valori per fascia
            pun_data.data[Fascia.ORARIA][ora] += prezzo_pun
            pun_data.data[fascia][ora] += prezzo_pun
            pz_data.data[Fascia.ORARIA][ora] += prezzo_pz
            pz_data.data[fascia][ora] += prezzo_pz

    # Divide ogni fascia per il numero di file (giorni) per completare la media
    for ora in range(24):
        for f in pun_data.data.keys():
            pun_data.data[f][ora] = pun_data.data[f][ora] / len(filelist)
    return pun_data, pz_data
