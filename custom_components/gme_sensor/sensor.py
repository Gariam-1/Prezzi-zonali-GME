"""Implementazione sensori di gse_sensor."""
...
async def async_setup_entry(
    ...
    entities.extend(
        PriceSensorEntity(coordinator, fascia, "pun") for fascia in PricesValues().value
    )
    entities.extend(
        PriceSensorEntity(coordinator, fascia, "prezzo_zonale") for fascia in PricesValues().value
    )

    # Crea sensori aggiuntivi
    entities.append(FasciaSensorEntity(coordinator))
    entities.append(PrezzoFasciaSensorEntity(coordinator, "pun"))
    entities.append(PrezzoFasciaSensorEntity(coordinator, "zonale"))
...
class PriceSensorEntity(CoordinatorEntity, SensorEntity, RestoreEntity):
    """Sensore relativo al prezzo medio mensile per fasce."""

    def __init__(self, coordinator: PricesDataUpdateCoordinator, fascia: Fascia, type: str) -> None:
        ...
        match self.fascia:
            case Fascia.MONO:
                self.entity_id = ENTITY_ID_FORMAT.format(f"{type}_mono_orario")
            case Fascia.F1:
                self.entity_id = ENTITY_ID_FORMAT.format(f"{type}_fascia_f1")
            case Fascia.F2:
                self.entity_id = ENTITY_ID_FORMAT.format(f"{type}_fascia_f2")
            case Fascia.F3:
                self.entity_id = ENTITY_ID_FORMAT.format(f"{type}_fascia_f3")
            case Fascia.F23:
                self.entity_id = ENTITY_ID_FORMAT.format(f"{type}_fascia_f23")
            case Fascia.ORARIA:
                self.entity_id = ENTITY_ID_FORMAT.format(f"{type}_orario")
            case _:
                self.entity_id = None
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        # Differenzia tra PUN e prezzo zonale
        if self.type == "pun":
            prices_data = self.coordinator.pun_data
            prices_values = self.coordinator.pun_values
        else:
            prices_data = self.coordinator.pz_data
            prices_values = self.coordinator.pz_values

        if self.fascia != Fascia.F23:
            # Tutte le fasce tranne F23
            if len(prices_data.data[self.fascia]) > 0:
                # Ci sono dati, sensore disponibile
                self._available = True
                self._native_value = prices_values.value[self.fascia]
            else:
                # Non ci sono dati, sensore non disponibile
                self._available = False

        elif (
            len(prices_data.data[Fascia.F2])
            and len(prices_data.data[Fascia.F3])
        ) > 0:
    ...
    def name(self) -> str | None:
        ...
        type = "PUN" if self.type == "pun" else "Prezzo zonale"
        if self.fascia == Fascia.MONO:
            return f"{type} mono-orario"
        elif self.fascia == Fascia.ORARIA:
            return f"{type} orario ({datetime.now().hour})"
        if self.fascia:
            return f"{type} fascia {self.fascia.value}"
        return None
...
class PrezzoFasciaSensorEntity(CoordinatorEntity, SensorEntity, RestoreEntity):
    ...
    def __init__(self, coordinator: PricesDataUpdateCoordinator, type: str) -> None:
        """Inizializza il sensore."""
        super().__init__(coordinator)
        ...
        # PUN o prezzo zonale
        self.type = "PUN" if type == "pun" else type

        # ID univoco sensore basato su un nome fisso
        self.entity_id = ENTITY_ID_FORMAT.format(f"prezzo_{type}_fascia_corrente")
        self._attr_unique_id = self.entity_id
        self._attr_has_entity_name = True
        ...
        self._available = False
        self._native_value = 0
        self._friendly_name = f"Prezzo {self.type} fascia corrente"
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self.coordinator.fascia_corrente is not None:
            self._available = (
                len(prices_data.data[self.coordinator.fascia_corrente]) > 0
            )
            self._native_value = prices_values.value[
                self.coordinator.fascia_corrente
            ]
            self._friendly_name = (
                f"Prezzo {self.type} fascia corrente ({self.coordinator.fascia_corrente.value})"
            )
        else:
            self._available = False
            self._native_value = 0
